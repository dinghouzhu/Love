...
import { getVideoList } from "../../actions/discover"
...
class DiscoverListView extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        setTimeout(() => {
            this.setState({
                dataSource: this.state.dataSource.cloneWithRows(store.getState().videoListView.VideoList),
                isLoading: false
            })
        }, 700);
    }
    ...
    render() {
        const row = (rowData, sectionID, rowID) => {
            return (
                ...
                <div key={rowID} className={discoverCss.videoDiv}>
                    <video className={discoverCss.VideoPlay} controls src={rowData.data.urlInfo.url}></video>
                    <span className={discoverCss.videoSpan}>{rowData.data.title}</span>
                    <img className={discoverCss.videoCreateAvatar} src={rowData.data.creator.avatarUrl} alt="" />
                </div>
            );
        };
